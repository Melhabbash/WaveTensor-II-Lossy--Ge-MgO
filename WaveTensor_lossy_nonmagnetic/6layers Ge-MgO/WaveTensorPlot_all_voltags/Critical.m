...
for m=1: number_of_points
    if (refractive_index_Air > refractive_index(m,1))
        Critical_Angle_0(m,1)=asin(refractive_index(m,1)/refractive_index_Air)*180/pi;
    end
end

for m=1: number_of_points
    for n=1:number_of_layers
            if(refractive_index(m,n)>refractive_index(m,n+1))
                Critical_Angle(m,n)=asin(refractive_index(m,n+1)/refractive_index(m,n))*180/pi;
            end
    end
end
...
if (refractive_index_Air > refractive_index(m,1))
    ...
    xticks([0 30 Critical_Angle_0(m,1) 90 ]);
    xticklabels({'0^o','30^o',['  \theta_c:' num2str(Critical_Angle_0(m,1)) '^o'] , '90^o'});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				...
				for n=2:number_of_layers
					...
					%     xticks([0 30 Critical_Angle(m,n-1) 90 ]);
					%     xticklabels({'0^o','30^o', num2str(Critical_Angle(m,n-1))  , '90^o'});
				...
				% 
				%     xticks([0 30 Critical_Angle(m,n-1) 90 ]);
				%     xticklabels({'0^o','30^o',[' \theta_B(theoretical):' num2str(Critical_Angle(m,n-1)) '^o'] , '90^o'});
				...
				%     xticks([0 30 Critical_Angle(m,n-1) 90 ]);
				%     xticklabels({'0^o','30^o','60^o' , '90^o'});
...
if (refractive_index(m,number_of_layers) > refractive_index(m,number_of_layers+1))
...
%     title({[ txt1 txt3  num2str(number_of_layers) ' and vacuum' ];  txt4} );
    ...
    txt = ['\leftarrow r_T_E=' num2str(rs_m) ];
    text(theta(jl,:,number_of_layers),rs_m,txt,'FontSize',9)   
    xticks([0 20 30 Critical_Angle(m,number_of_layers) 90 ]);
    xticklabels({'0^o','20^o','30^o',['  \theta_c:' num2str(Critical_Angle(m,number_of_layers)) '^o'] , '90^o'});
...
%     title({[ txt1 txt3  num2str(number_of_layers) ' and vacuum'  ]; txt4} );
    ...
    txt = ['\leftarrow r_T_M^2= r_T_E^2=' num2str(rprp_min) ];
...
%     title({[ txt1 txt3  num2str(number_of_layers) ' and vacuum' ]; txt4} );
    ...
    txt = ['\leftarrow t_T_M=' num2str(tp_m) ];
    text(theta(jl,:,number_of_layers),tp_m ,txt,'FontSize',9)
    txt = ['\leftarrow t_T_E=' num2str(ts_m) ];
...
%     title({[ txt1 txt3  num2str(number_of_layers) ' and vacuum'  ]; txt4} );
    ...
    txt = ['\leftarrow t_T_M^2=' num2str(tptp_m) ];
    text(theta(jjl,:,number_of_layers),tptp_m,txt,'FontSize',9)

    txt = ['\leftarrow t_T_E^2=' num2str(tsts_m) ];
