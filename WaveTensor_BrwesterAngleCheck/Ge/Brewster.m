function [Brewster_Angle_0, Refracted_Angle_due_to_Brewster_Angle ] = Brewster(tag, number_of_points,number_of_layers,refractive_index,rp,rs,tp,ts,theta_0,theta )

refractive_index_Air=1;

for m=1: number_of_points
Brewster_Angle_0(m,1)=atan(real (refractive_index(m,1))/refractive_index_Air)*180/pi;
end


for m=1: number_of_points
    for n=1:number_of_layers
Refracted_Angle_due_to_Brewster_Angle(m,n)=asin(sin(Brewster_Angle_0(m,1)*pi/180)/real(refractive_index(m,n)))*180/pi;
    end
end


 Brewster_Angle_0
 Refracted_Angle_due_to_Brewster_Angle
...
txt = ['\leftarrow r_T_E=' num2str(abs(rs_min))];
...
txt = ['\leftarrow t_T_M= ' num2str(abs(tp_min)) ];
text(theta_0(j),tp_min ,txt,'FontSize',10)
txt = ['\leftarrow t_T_E= ' num2str(abs(ts_min)) ];
...
txt = ['\leftarrow r_T_M^2 ' num2str(abs(rprp_min))];
text(theta_0(j),rprp_min,txt,'FontSize',10)
txt = ['\leftarrow r_T_E^2 ' num2str(abs(rsrs_min))];
...
txt = ['\leftarrow t_T_M^2=' num2str(abs(tptp_min)) ];
text(theta_0(j),tptp_min,txt,'FontSize',10)

txt = ['\leftarrow t_T_E^2=' num2str(abs(tsts_min)) ];
...
%         title({[ txt1 txt3 num2str(n-1) ' and ' txt3 num2str(n)  ];  txt4} );
        ...
        xticks([0 30 Refracted_Angle_due_to_Brewster_Angle(m,n-1) 90 ]);
        xticklabels({'0^o','30^o', ['\theta_B: ' num2str(Refracted_Angle_due_to_Brewster_Angle(m,n-1)) '^o']  , '90^o'});

%         xticklabels({'0^o','30^o', [' \theta_B(theoretical):' num2str(Refracted_Angle_due_to_Brewster_Angle(m,n-1)) '^o']  , '90^o'});
...
%         title({[ txt1 txt3 num2str(n-1) ' and ' txt3 num2str(n)  ];  txt4} );
        ...
        txt = '\leftarrow t_T_M ';
        text(theta(3500,:,n-1),abs(tp(3500,:,n)),txt,'FontSize',10)
        txt = '\leftarrow t_T_E ';
        text(theta(5000,:,n-1),abs(ts(5000,:,n)),txt,'FontSize',10)
...
%     title({[ txt1 txt3  num2str(number_of_layers) ' and vacuum' ];  txt4} );
    ...
    rp_min=(rp(1,:,number_of_layers+1));
    a=size(theta);
    for i=2:a(1)
    ...
    txt = ['\leftarrow r_T_M=' num2str(abs(rp_mlast))];
    text(theta(jl,:,number_of_layers),rp_mlast,txt,'FontSize',10)
    txt = ['\leftarrow r_T_E=' num2str(abs(rs_mlast))];
    text(theta(jl,:,number_of_layers),rs_mlast,txt,'FontSize',10)

     
    xticks([0 20  Refracted_Angle_due_to_Brewster_Angle(m,number_of_layers) 90 ]);
    xticklabels({'0^o','20^o',['\theta_B: ' num2str(Refracted_Angle_due_to_Brewster_Angle(m,number_of_layers)) '^o'] , '90^o'});
%     xticklabels({'0^o','20^o',[' \theta_B(theoretical):' num2str(Refracted_Angle_due_to_Brewster_Angle(m,number_of_layers)) '^o'] , '90^o'});
...
%     title({[ txt1 txt3  num2str(number_of_layers) ' and vacuum'  ]; txt4} );
    ...
    rprp_min=(rp(1,:,number_of_layers+1).*rp(1,:,number_of_layers+1));
    a=size(theta);
    for i=2:a(1)
    ...
    y1=get(gca,'ylim');
     ...
     txt = ['\leftarrow r_T_M^2 ' num2str(abs(rprp_mlast))];
     text(theta(jjl,:,number_of_layers),rprp_mlast,txt,'FontSize',10)
     txt = ['\leftarrow r_T_E^2 ' num2str(abs(rsrs_mlast))];
...
%     xlim([0 50])
    ...
    txt = ['\leftarrow t_T_M= ' num2str(abs(tp_mlast)) ];
    text(theta(jl,:,number_of_layers),tp_mlast ,txt,'FontSize',10)
    txt = ['\leftarrow t_T_E= ' num2str(abs(ts_mlast)) ];
...
%     xlim([0 50])
    ...
    y1=get(gca,'ylim');
     ...
     txt = ['\leftarrow t_T_M^2=' num2str(abs(tptp_mlast)) ];
     text(theta(jjl,:,number_of_layers),tptp_mlast,txt,'FontSize',10)

     txt = ['\leftarrow t_T_E^2=' num2str(abs(tsts_mlast)) ];
